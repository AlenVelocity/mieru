...
const schema = z.object({
    projectName: z.string().min(5).describe(
        'The name of the project based on the user input'
    ),
    response: z.string().min(10).describe(
        'AI\'s response to the user, describing how the project will be planned'
    ),
    steps: z.array(z.string()).min(1).describe(
        'The steps that the AI will take to plan the project'
    )
})
